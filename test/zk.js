...
test('ZK-SNARK proof generation and verification', async function (t) {
  t.plan(5); // Increased to account for signature verification
  ...
  const isSignatureValid = Keychain.verify(message, signature, signer.publicKey);
  t.ok(isSignatureValid, 'Signature should be valid');
